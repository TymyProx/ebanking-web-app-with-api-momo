...
export default function StatementsPage() {
  ...
  const handleGenerateStatement = async () => {
    ...
    if (format === "pdf") {
      // PDF is generated by the backend (proxied via /api/pdf/statement)
      try {
        ...
        if (!res.ok) {
          ...
          setErrorMessage(err?.error || "Erreur lors de la génération du relevé PDF.")
          return
        }
        ...
        setErrorMessage(e instanceof Error ? e.message : "Erreur lors de l'affichage du relevé PDF.")
        return
      }
    } else if (format === "excel") {
      // Récupérer les soldes depuis STTMS pour la génération du relevé Excel
      const sttmsBalances = await getStatementBalancesFromSTTMS(
        selectedAccount.number, // acno = numéro de compte
        startDate, // dateDepart
        endDate, // dateFin
  ...
  const handleDownloadPDF = () => {
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}))
          setErrorMessage(err?.error || "Erreur lors du téléchargement du relevé PDF.")
          return
        }
        ...
        setErrorMessage(e instanceof Error ? e.message : "Erreur lors du téléchargement du relevé PDF.")
      }
    })()
  }
  ...
  // NOTE: la génération PDF côté frontend (jsPDF) a été supprimée.
