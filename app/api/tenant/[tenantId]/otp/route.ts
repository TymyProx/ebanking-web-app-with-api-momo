...
export async function GET(request: Request, { params }: { params: { tenantId: string } }) {
  ...
  if (!tenantId) {
    return NextResponse.json(
      {
        success: false,
        error: "Tenant ID requis pour lister les OTP",
      },
      { status: 400 },
    )
  }
  ...
  try {
    const backendResponse = await fetch(`${API_BASE_URL}/tenant/${tenantId}/otp`, {
      method: "GET",
      headers: {
    ...
    console.error("[OTP Proxy] list error:", error)
    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Erreur lors de la récupération des OTP",
      },
      { status: 500 },
    )
  }
