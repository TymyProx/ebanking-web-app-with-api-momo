...
export async function POST(request: Request, { params }: { params: { tenantId: string } }) {
  ...
  if (!tenantId) {
    return NextResponse.json(
      {
        success: false,
        error: "Tenant ID requis pour générer un OTP",
      },
      { status: 400 },
    )
  }
  ...
  try {
    ...
    const backendResponse = await fetch(`${API_BASE_URL}/tenant/${tenantId}/otp/generate`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    console.error("[OTP Proxy] generate error:", error)
    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Erreur lors de la génération de l'OTP",
      },
      { status: 500 },
    )
  }
