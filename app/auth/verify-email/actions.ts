...
async function completeNewClientSignup(pendingData: any, token: string, password: string, hasPendingData: boolean) {
...
export async function completeSignup(token: string, password: string, emailFallback?: string) {
  try {
    ...
    console.log("[v0] Routing to appropriate signup function...")
    console.log("[v0] isExistingClient:", isExistingClient)

    if (isExistingClient) {
      console.log("[v0] Calling completeExistingClientSignup()...")
      return await completeExistingClientSignup(pendingData, token, password)
    } else {
      console.log("[v0] Calling completeNewClientSignup()...")
      return await completeNewClientSignup(pendingData, token, password, hasPendingData)
    }
  } catch (error: any) {
