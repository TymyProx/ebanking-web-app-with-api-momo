...
export default function ServiceRequestsPage() {
  ...
  const generateRequestPDF = async (request: any) => {
    ...
    const createUnifiedTable = (startY: number, data: string[][]) => {
      ...
      doc.text("DÉTAILS DE LA DEMANDE", 105, currentY + 8, { align: "center" })
      currentY += rowHeight
    ...
    doc.setFontSize(24)
    doc.setFont("helvetica", "bold")
    doc.text("DEMANDE DE SERVICE BANCAIRE", 105, 20, { align: "center" })
    ...
    doc.setFontSize(18)
    doc.setFont("helvetica", "bold")
    doc.text("BANQUE NATIONALE DE GUINÉE", 20, 55)
    ...
    if (request.type === "checkbook") {
      requestData = [
        ["Type de demande", request.typeName],
        ["Référence", request.reference],
        ["Statut", request.status],
        ["Date de soumission", new Date(request.submittedAt).toLocaleDateString("fr-FR")],
        ["Compte", request.account],
        ["Nombre de chéquiers", request.details?.nbrechequier?.toString() || "N/A"],
        ["Nombre de feuilles", request.details?.nbrefeuille?.toString() || "N/A"],
        ["Commentaire", request.details?.commentaire || "Aucun commentaire"],
      ]
      ...
      requestData = [
        ...
        ["Date de soumission", new Date(request.submittedAt).toLocaleDateString("fr-FR")],
        ["Compte", request.account],
        ["Nom du demandeur", request.details?.applicantName || "N/A"],
        ["Montant du crédit", request.details?.creditAmount ? `${request.details.creditAmount} GNF` : "N/A"],
        ["Durée", request.details?.durationMonths ? `${request.details.durationMonths} mois` : "N/A"],
        ["Objet du crédit", request.details?.purpose || "N/A"],
      ]
    } else {
      // Demande générique
      requestData = [
    ...
    yPos = createUnifiedTable(yPos, requestData)
    yPos += 20
    ...
    doc.text("Ce document atteste de votre demande de service bancaire.", 105, yPos + 8, {
      align: "center",
    })
    doc.text("Conservez-le précieusement pour vos échanges avec la banque.", 105, yPos + 14, {
      align: "center",
    })
    ...
    doc.text(`Réf: ${request.reference}-${Date.now()}`, 20, 285)

    const fileName = `Demande_${request.reference}_${new Date().toISOString().split("T")[0]}.pdf`
    doc.save(fileName)
  }

  const downloadRequestPDF = async (request: any) => {
    if (!request) return
    ...
    try {
      ...
      ${
        request.type === "checkbook"
          ? `
      Nombre de chéquiers: ${request.details?.nbrechequier || "N/A"}
      Nombre de feuilles: ${request.details?.nbrefeuille || "N/A"}
      Commentaire: ${request.details?.commentaire || "Aucun commentaire"}
      `
          : ""
      }
    ...
    `
      ...
      a.href = url
      a.download = `Demande_${request.reference}_${new Date().toISOString().split("T")[0]}.txt`
          ...
          {isLoadingDetails ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-end space-x-2 pt-4 border-t">
                ...
                <Button
                  onClick={() => downloadRequestPDF(allRequests.find((r) => r.id === selectedRequestDetails?.id))}
                >
                  <Download className="w-4 h-4 mr-2" />
                  Télécharger
                </Button>
